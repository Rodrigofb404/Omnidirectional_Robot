...
void timer1_PWM_invert_mode (int8_t mode, int8_t start_OC1B) {
    ...
    if (mode == 0) {
        ...
        if (start_OC1B >= 0) TCCR1A |= (1 << COM1B1);
        
    } else if (mode == 1) {
        TCCR1A |= (1 << COM1A0) | (1 << COM1A1);
        
        if (start_OC1B >= 0) TCCR1A |= (1 << COM1B0) | (1 << COM1B1);
    }
}
...
void timer1_PWM_value (uint16_t PWM_valueA, uint16_t PWM_valueB) {
    if (OCR1A + PWM_valueA >= 0 && OCR1B + PWM_valueB >= 0) {
        OCR1A += PWM_valueA;
        OCR1B += PWM_valueB;
    }
}
...
void config_CTC1 (int8_t mode) {
    ...
    switch (mode) {
        case 0:
            TIMSK1 |= (1 << OCIE1A);
            OCR1A = calc_compare_value(0.03, 16000000, 8);
            break;
        case 1:
            TIMSK1 |= (1 << OCIE1B);
            OCR1B = calc_compare_value(0.03, 16000000, 8);
            break;
        default:
            break;
    }
